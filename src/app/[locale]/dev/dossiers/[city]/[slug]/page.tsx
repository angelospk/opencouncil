...
const SOURCING_NOTE: Record<string, string> = {
  ...
  moderate:
    "Μέτριες εξωτερικές πηγές: σχετική νομοθεσία και ο φορέας διαχείρισης.",
  minimal:
    "Ελάχιστες εξωτερικές πηγές — κυρίως το εσωτερικό χρονολόγιο του συμβουλίου.",
  none: "Χωρίς εξωτερικές πηγές — μόνο το εσωτερικό χρονολόγιο του συμβουλίου.",
};

export function generateStaticParams() {
  return getDossiers().map((dossier) => ({
    city: dossier.cityId,
    slug: dossier.slug,
  }));
}
...
export default async function DossierPage(props: {
  ...
  const { locale, city, slug } = await props.params;
  const dossier = getDossierBySlug(city, slug);
  if (!dossier) {
    notFound();
  }
  ...
  const entitySources = Array.from(
    new Set(dossier.entries.flatMap((e) => e.citationUrls)),
  ).slice(0, 8);
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
      ...
      <Link
        ...
        className="ml-4 inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"
      >
        Φάκελοι πόλης
      </Link>
      ...
      <header className="mt-4">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          ...
          <span className="text-xs text-muted-foreground">
            {dossier.entries.length} καταχωρήσεις · {dossier.minutesDiscussed}′
            συζήτησης
          </span>
        </div>
      </header>
      ...
      <ol className="relative border-l border-border pl-6">
        {dossier.entries.map((entry, i) => (
          <li
            key={entry.subjectId}
            className={i === dossier.entries.length - 1 ? "" : "mb-8"}
          >
                  ...
                  >
                    ...
                    <span>
                      <span className="font-semibold">
                        Απόφαση (Διαύγεια {entry.decision.ada})
                      </span>
                      <br />
